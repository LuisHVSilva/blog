...
export class ExportSnapshotCli {
    static async execute(argv = process.argv.slice(2)) {
        ...
        let snapshot;
        try { await database.connect(); snapshot = await EditorialComposition.create(database.getSequelize(), config.publicSiteUrl.replace(/\/$/u, '')).exportSnapshot.execute(); }
...
if (require.main === module) ExportSnapshotCli.execute().then((value) => process.stdout.write(JSON.stringify({revision: value.revision, articles: value.articles.length}) + '\n')).catch((error: unknown) => { console.error(error instanceof Error ? error.message : 'Export failed.'); process.exitCode = 1; });
