...
export async function execute(argv = process.argv.slice(2)) {
    ...
    const result = validateContent(await parseContentRoot(path.resolve(argv[rootIndex + 1]!)));
    process.stdout.write(format === 'json' ? JSON.stringify(result) + '\n' : `${result.valid ? 'valid' : 'invalid'}: ${result.errors.length} error(s)\n`);
    if (!result.valid) process.exitCode = 2;
    return result;
}
if (require.main === module) execute().catch((error: unknown) => { console.error(error instanceof Error ? error.message : 'Content validation failed.'); process.exitCode = 2; });
