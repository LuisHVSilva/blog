...
export const catalogSchema = z.object({
    ...
    authors: z.array(z.object({
        ...
        links: z.array(z.url().refine((value) => ['http:', 'https:'].includes(new URL(value).protocol)))
    }).strict()),
    ...
    operations: z.array(z.object({
        kind: z.enum(['unpublish', 'archiveTranslation', 'archive', 'restore', 'restoreTranslation']),
        articleId: z.uuid(),
    ...
    const unique = (values: readonly (string | number)[], label: string): void => {
        if (new Set(values).size !== values.length) {
            context.addIssue({code: 'custom', message: `Repeated ${label}.`});
        }
    };

    unique(catalog.authors.map((item) => item.id), 'author');
    unique(catalog.authors.map((item) => item.profileSlug), 'profile slug');
    ...
    const tags = new Set(catalog.tags.map((item) => item.id));
    const articles = new Set(catalog.articles.map((item) => item.id));

    for (const article of catalog.articles) {
        if (!authors.has(article.authorId) || article.tagIds.some((id) => !tags.has(id))) {
            context.addIssue({code: 'custom', message: 'Missing author or tag reference.'});
        }
    ...
    for (const series of catalog.series) {
        unique(series.members.map((member) => member.articleId), 'series member');
        unique(series.members.map((member) => member.position), 'series position');
        if (series.members.some((member) => !articles.has(member.articleId))) {
            context.addIssue({code: 'custom', message: 'Missing series article.'});
        }
    }
