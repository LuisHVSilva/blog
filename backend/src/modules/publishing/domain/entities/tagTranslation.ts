...
export type TagTranslationProps = Readonly<{
    tagId: string;
    locale: Locale;
    name: string;
    slug: string;
...
export class TagTranslation extends EntityBase<TagTranslationProps & {readonly id: string}, TagTranslation> {
    constructor(props: TagTranslationProps) {
        super({...props, id: JSON.stringify([props.tagId, props.locale])});
        Object.freeze(this);
    }

    protected recreate(props: TagTranslationProps): TagTranslation {
        return new TagTranslation(props);
    }
