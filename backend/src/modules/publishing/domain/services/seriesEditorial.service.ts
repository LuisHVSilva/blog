...
export class SeriesEditorialService implements ISeriesEditorialService {
    ...
    async validate(values: EditorialCatalog['series']): Promise<void> {
        for (const value of values) {
            ...
            for (const translation of value.translations) {
                if ((await this.translations.find({locale: translation.locale, slug: translation.slug})).some((owner) => owner.toData().seriesId !== value.id)) throw new PublishingConflictError('Taxonomy slug is already reserved.');
    ...
    async writeMembers(values: EditorialCatalog['series']): Promise<void> {
        for (const series of values) {
            ...
            for (const member of series.members) await this.members.save(new SeriesArticle({seriesId: series.id, ...member}));
        }
    }
}
