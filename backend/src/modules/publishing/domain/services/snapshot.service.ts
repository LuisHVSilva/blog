...
import type {IEditorialUnitOfWork, IEditorialClock} from './editorialRuntime.interface';
import type {PublicSnapshot, TagSummary, SeriesDetail} from '../public-content.types';

export class SnapshotService implements ISnapshotService {
    constructor(private readonly scope: IEditorialUnitOfWork, private readonly articles: IArticleService,
    ...
    async export(): Promise<PublicSnapshot> {
        return await this.scope.execute(async () => {
            ...
            if (!revision.revision) throw new Error('Cannot export an unpublished catalogue revision.');
            ...
            for (const locale of ['pt-BR', 'en'] as const) {
                ...
                for (const summary of await this.seriesService.list(locale)) {
                    ...
                    if (!detail) throw new Error('Series disappeared within the snapshot.');
                    series.push(detail);
                }
            }
            ...
            for (const path of await this.paths.redirects()) {
                ...
                if (article) redirects.push({from: `/${path.locale}/articles/${path.slug}`, to: `/${article.locale}/articles/${article.slug}`, status: 308});
            }
            const lastmod = revision.updatedAt.toISOString();
            return {schemaVersion: 1, revision: revision.revision, generatedAt: this.clock.now().toISOString(), siteOrigin: this.siteOrigin, articles, tags, series, redirects,
