...
export class EditorialTranslationService implements IEditorialTranslationService {
    ...
    async findById(id: string) {
        if (!EditorialRules.isUuid(id)) {
            throw new PublishingValidationError('Invalid entity ID.');
        }
        return await this.repository.findById(id);
    }

    async save(entity: ArticleTranslation) {
        return await this.repository.save(ArticleTranslation.rehydrate(entity.getProps()));
    }

    async findByArticleLocale(articleId: string, locale: Locale) {
        if (!EditorialRules.isLocale(locale)) {
    ...
    async listForArticle(articleId: string) {
        return await this.repository.listForArticle(articleId);
    }
}
