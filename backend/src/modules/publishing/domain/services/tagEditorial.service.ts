...
export class TagEditorialService implements ITagEditorialService {
    constructor(private readonly repository: ITagRepository, private readonly translations: ITagTranslationRepository) {}
    async validate(values: EditorialCatalog['tags']): Promise<void> {
        for (const value of values) {
            if ((await this.repository.find({key: value.key})).some((owner) => owner.id !== value.id)) throw new PublishingConflictError('Taxonomy key is already reserved.');
            for (const translation of value.translations) {
                if ((await this.translations.find({locale: translation.locale, slug: translation.slug})).some((owner) => owner.toData().tagId !== value.id)) throw new PublishingConflictError('Taxonomy slug is already reserved.');
                const previous = (await this.translations.find({tagId: value.id, locale: translation.locale}))[0]?.toData();
                if (previous?.publishedOnce && previous.slug !== translation.slug) throw new PublishingConflictError('TAXONOMY_SLUG_IMMUTABLE');
            }
        }
    }
    async write(values: EditorialCatalog['tags'], now: Date): Promise<void> {
        ...
        for (const value of values) {
            ...
            for (const translation of value.translations) {
                ...
                await this.translations.save(new TagTranslation({tagId: value.id, ...translation, description: translation.description ?? null, publishedOnce: before?.publishedOnce || translation.status === 'published'}));
            }
        }
    }
}
