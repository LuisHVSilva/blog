...
import type {
    PublicationEdition,
    PublicationEditionProps
} from '../../../modules/publishing/domain/entities/publicationEdition';
import type {
    IPublicationEditionRepository
} from '../../../modules/publishing/domain/repositories/publicationEdition.repository.interface';
...
export class PublicationEditionPersistence implements IPublicationEditionRepository {
    ...
    constructor(database: Sequelize, private readonly context: IPersistenceContext) {
        this.model = database.getRepository(PublicationEditionModel);
    }
    ...
    async find(filter: Partial<PublicationEditionProps>, lock = false): Promise<readonly PublicationEdition[]> {
        ...
        const rows: PublicationEditionModel[] = await this.model.findAll({
            where: filter as WhereOptions<InferAttributes<PublicationEditionModel>>,
            transaction,
    ...
    async save(entity: PublicationEdition): Promise<void> {
        await this.model.create(Mapper.toPersistence(entity), {
