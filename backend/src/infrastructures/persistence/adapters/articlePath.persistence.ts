...
import type {
    IArticlePathRepository
} from '../../../modules/publishing/domain/repositories/articlePath.repository.interface';
...
export class ArticlePathPersistence implements IArticlePathRepository {
    ...
    constructor(
        ...
        this.model = database.getRepository(ArticlePathModel);
    }

    async find(filter: Partial<ArticlePathProps>, lock = false): Promise<readonly ArticlePath[]> {
        const transaction: Transaction | undefined = lock
            ? this.context.requireTransaction()
            : this.context.getTransaction();
        const rows: ArticlePathModel[] = await this.model.findAll({
            where: filter as WhereOptions<InferAttributes<ArticlePathModel>>,
            transaction,
    ...
    async save(entity: ArticlePath): Promise<void> {
        ...
        const [row] = await this.model.findOrCreate({
            where: {locale: values.locale, slug: values.slug},
            defaults: values,
            transaction
        });

        // Never transfer path ownership, even if another writer claimed a previously empty path.
        if (row.translationId === values.translationId) {
            await row.update({kind: values.kind}, {transaction});
        }
    }
