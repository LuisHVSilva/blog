...
import type {
    SeriesTranslation,
    SeriesTranslationProps
} from '../../../modules/publishing/domain/entities/seriesTranslation';
import type {
    ISeriesTranslationRepository
} from '../../../modules/publishing/domain/repositories/seriesTranslation.repository.interface';
...
export class SeriesTranslationPersistence implements ISeriesTranslationRepository {
    ...
    constructor(database: Sequelize, private readonly context: IPersistenceContext) {
        this.model = database.getRepository(SeriesTranslationModel);
    }
    ...
    async find(filter: Partial<SeriesTranslationProps>, lock = false): Promise<readonly SeriesTranslation[]> {
        ...
        const rows: SeriesTranslationModel[] = await this.model.findAll({
            where: filter as WhereOptions<InferAttributes<SeriesTranslationModel>>,
            transaction,
