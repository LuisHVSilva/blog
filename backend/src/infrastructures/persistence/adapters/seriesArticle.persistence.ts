...
import type {ISeriesArticleRepository} from '../../../modules/publishing/domain/repositories/seriesArticle.repository.interface';
export class SeriesArticlePersistence implements ISeriesArticleRepository {
    ...
    constructor(database: Sequelize, private readonly context: IPersistenceContext) { this.model = database.getRepository(SeriesArticleModel); }
    async find(filter: Partial<SeriesArticleProps>, lock = false): Promise<readonly SeriesArticle[]> {
        ...
        const rows = await this.model.findAll({where: filter as WhereOptions<InferAttributes<SeriesArticleModel>>, transaction, ...(lock && transaction ? {lock: transaction.LOCK.UPDATE} : {})});
    ...
    async remove(filter: Partial<SeriesArticleProps>): Promise<void> {
        ...
        await this.model.destroy({where: filter as WhereOptions<InferAttributes<SeriesArticleModel>>, transaction: this.context.requireTransaction()});
    }
}
