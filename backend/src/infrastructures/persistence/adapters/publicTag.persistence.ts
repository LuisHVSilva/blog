...
import type {IPublicTagRepository} from '../../../modules/publishing/domain/repositories/publicTag.repository.interface';
...
export class PublicTagPersistence implements IPublicTagRepository {
    ...
    async list(locale: Locale): Promise<readonly PublishedTag[]> {
        ...
        return rows.map((row) => new PublishedTag({id: row.id, key: row.key, locale, name: row.name, slug: row.slug, ...(row.description === null ? {} : {description: row.description}), articleCount: PersistenceNumbers.count(row.article_count), canonical: `${this.siteOrigin}/${locale}/tags/${row.slug}`}));
    }
}
