...
} from '../../../modules/publishing/domain/repositories/authorProfile.repository.interface';
...
export class AuthorProfilePersistence implements IAuthorProfileRepository {
    ...
    constructor(database: Sequelize, private readonly context: IPersistenceContext) {
        this.model = database.getRepository(AuthorProfileModel);
    }
    ...
    async find(filter: Partial<AuthorProfileProps>, lock = false): Promise<readonly AuthorProfile[]> {
        ...
        const rows: AuthorProfileModel[] = await this.model.findAll({
            where: filter as WhereOptions<InferAttributes<AuthorProfileModel>>,
            transaction,
