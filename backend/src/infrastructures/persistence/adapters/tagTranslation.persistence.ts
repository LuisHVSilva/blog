...
import type {ITagTranslationRepository} from '../../../modules/publishing/domain/repositories/tagTranslation.repository.interface';
export class TagTranslationPersistence implements ITagTranslationRepository {
    ...
    constructor(database: Sequelize, private readonly context: IPersistenceContext) { this.model = database.getRepository(TagTranslationModel); }
    async find(filter: Partial<TagTranslationProps>, lock = false): Promise<readonly TagTranslation[]> {
        ...
        const rows = await this.model.findAll({where: filter as WhereOptions<InferAttributes<TagTranslationModel>>, transaction, ...(lock && transaction ? {lock: transaction.LOCK.UPDATE} : {})});
    ...
    async remove(filter: Partial<TagTranslationProps>): Promise<void> {
        ...
        await this.model.destroy({where: filter as WhereOptions<InferAttributes<TagTranslationModel>>, transaction: this.context.requireTransaction()});
    }
}
