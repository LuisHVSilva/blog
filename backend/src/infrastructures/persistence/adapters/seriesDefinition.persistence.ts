...
import type {
    SeriesDefinition,
    SeriesDefinitionProps
} from '../../../modules/publishing/domain/entities/seriesDefinition';
import type {
    ISeriesDefinitionRepository
} from '../../../modules/publishing/domain/repositories/seriesDefinition.repository.interface';
...
export class SeriesDefinitionPersistence implements ISeriesDefinitionRepository {
    ...
    constructor(database: Sequelize, private readonly context: IPersistenceContext) {
        this.model = database.getRepository(SeriesModel);
    }
    ...
    async find(filter: Partial<SeriesDefinitionProps>, lock = false): Promise<readonly SeriesDefinition[]> {
        ...
        const rows: SeriesModel[] = await this.model.findAll({
            where: filter as WhereOptions<InferAttributes<SeriesModel>>,
            transaction,
