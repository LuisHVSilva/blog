...
export class ArticlePersistence implements IArticleRepository {
    ...
    async findById(id: string): Promise<Article | null> {
        const transaction: Transaction | undefined = this.context.getTransaction();

        const row: ArticleModel | null = await this.model.findByPk(id, {
            transaction,
    ...
    async save(entity: Article): Promise<Article> {
        ...
        const saved: Article | null = await this.findById(entity.id);

        if (!saved) {
            throw new Error('Persisted article was not found.');
        }
