...
export class TagPersistence implements ITagRepository {
    ...
    constructor(database: Sequelize, private readonly context: IPersistenceContext) { this.model = database.getRepository(TagModel); }
    async find(filter: Partial<TagProps>, lock = false): Promise<readonly Tag[]> {
        ...
        const rows = await this.model.findAll({where: filter as WhereOptions<InferAttributes<TagModel>>, transaction, ...(lock && transaction ? {lock: transaction.LOCK.UPDATE} : {})});
    ...
    async remove(filter: Partial<TagProps>): Promise<void> {
        ...
        await this.model.destroy({where: filter as WhereOptions<InferAttributes<TagModel>>, transaction: this.context.requireTransaction()});
    }
}
