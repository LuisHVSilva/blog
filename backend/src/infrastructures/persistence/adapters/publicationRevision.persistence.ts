...
import type {PublicationRevision, PublicationRevisionProps} from '../../../modules/publishing/domain/entities/publicationRevision';
import type {IPublicationRevisionRepository} from '../../../modules/publishing/domain/repositories/publicationRevision.repository.interface';
export class PublicationRevisionPersistence implements IPublicationRevisionRepository {
    ...
    constructor(database: Sequelize, private readonly context: IPersistenceContext) { this.model = database.getRepository(PublicationCurrentRevisionModel); }
    async find(filter: Partial<PublicationRevisionProps>, lock = false): Promise<readonly PublicationRevision[]> {
        ...
        const rows = await this.model.findAll({where: filter as WhereOptions<InferAttributes<PublicationCurrentRevisionModel>>, transaction, ...(lock && transaction ? {lock: transaction.LOCK.UPDATE} : {})});
    ...
    async remove(filter: Partial<PublicationRevisionProps>): Promise<void> {
        ...
        await this.model.destroy({where: filter as WhereOptions<InferAttributes<PublicationCurrentRevisionModel>>, transaction: this.context.requireTransaction()});
    }
}
