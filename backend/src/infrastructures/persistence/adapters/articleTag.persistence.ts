...
} from '../../../modules/publishing/domain/repositories/articleTag.repository.interface';
...
export class ArticleTagPersistence implements IArticleTagRepository {
    ...
    constructor(database: Sequelize, private readonly context: IPersistenceContext) {
        this.model = database.getRepository(ArticleTagModel);
    }
    ...
    async find(filter: Partial<ArticleTagProps>, lock = false): Promise<readonly ArticleTag[]> {
        ...
        const rows: ArticleTagModel[] = await this.model.findAll({
            where: filter as WhereOptions<InferAttributes<ArticleTagModel>>,
            transaction,
