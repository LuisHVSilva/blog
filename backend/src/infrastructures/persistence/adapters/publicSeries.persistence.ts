...
import {PersistenceNumbers} from '../mappers/publicArticle.persistence.mapper';
import {visible} from '../queries/publicArticle.query';
import {PublishedSeries} from '../../../modules/publishing/domain/entities/publishedSeries';
import type {IPublicSeriesRepository} from '../../../modules/publishing/domain/repositories/publicSeries.repository.interface';
...
export class PublicSeriesPersistence implements IPublicSeriesRepository {
    constructor(private readonly sequelize: Sequelize, private readonly siteOrigin: string, private readonly transaction?: Transaction | IPersistenceContext) {}
    ...
    async list(locale: Locale): Promise<readonly PublishedSeries[]> {
        ...
        return rows.map((row) => new PublishedSeries({id: row.id, locale, slug: row.slug, title: row.title, description: row.description, ...(row.difficulty ? {difficulty: row.difficulty} : {}), articleCount: PersistenceNumbers.count(row.article_count), canonical: `${this.siteOrigin}/${locale}/series/${row.slug}`}));
    }
    async countMembers(seriesId: string): Promise<number> {
        const rows = await this.query<{total: string}>('SELECT count(*)::text total FROM series_articles WHERE series_id=:seriesId', {seriesId});
