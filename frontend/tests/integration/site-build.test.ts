...
it('E09: builds localized institutional pages with useful links and no placeholder controls', () => {
    ...
    try {
        ...
        expect(projects).toContain('Ainda não há projetos publicados.');
        expect(projects).toContain('<title>Projetos publicados | Stackcraft</title>');
        expect(projects).toContain('name="description" content="Projetos de software publicados e suas tecnologias."');
        expect(projects).toContain('rel="canonical" href="https://catalog.example.test/pt-BR/projects"');
        expect(projects).toContain('hreflang="en" href="https://catalog.example.test/en/projects"');
        expect(projects).toContain('name="robots" content="index, follow"');
